...
LV_IMG_DECLARE(img_magician_hat);
LV_IMG_DECLARE(img_magician_wand);
...
enum {
    ...
    kDetailsPadY_px = kHeaderPadY_px,
    kDetailsH_px    = 18,
    kDetailsW_px    = 65,
...
static const lv_point_t FWVerOrigin     = {.x = FWTitleOrigin.x, .y = FWTitleOrigin.y + kTextPadY_px };
...
OSD_Result_t Firmware_Draw(void* arg)
{
    ...
    if (_Ctx.pHatImgObj == NULL)
    {
        _Ctx.pHatImgObj = lv_img_create(pScreen);
        lv_img_set_src(_Ctx.pHatImgObj, &img_magician_hat);
        lv_obj_align_to(_Ctx.pHatImgObj, _Ctx.pChromaticImgObj,
                        LV_ALIGN_OUT_TOP_MID, 0, 2);
    }

    if (_Ctx.pWandImgObj == NULL)
    {
        _Ctx.pWandImgObj = lv_img_create(pScreen);
        lv_img_set_src(_Ctx.pWandImgObj, &img_magician_wand);
        lv_obj_align_to(_Ctx.pWandImgObj, _Ctx.pChromaticImgObj,
                        LV_ALIGN_OUT_LEFT_MID, -4, 2);
    }
    ...
    else 
    {
        ...
        if (_Ctx.pFWTitle == NULL) 
        {
            ...
            lv_label_set_text(_Ctx.pFWTitle, "CM:");
...
OSD_Result_t Firmware_OnTransition(void* arg)
{
    ...
    const lv_obj_t** ToDelete[] = {
        ...
        (const lv_obj_t**)&_Ctx.pHatImgObj,
        (const lv_obj_t**)&_Ctx.pWandImgObj,
...
OSD_Result_t Firmware_Initialize(void)
{
    ...
    lv_snprintf(_Ctx.ChromaticFWVerStr, sizeof(_Ctx.ChromaticFWVerStr), "%s", CONFIG_CHROMATIC_FW_VER_STR);
