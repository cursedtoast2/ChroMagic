...
#include "system/cart_backup_ui.h"
#include "system/pc_backup_mode.h"
#include "system/rom_browser_ui.h"
#include "osd.h"
...
static void CreateMenuBackups(void);
...
static void CreateMenuSystem(lv_obj_t *const pScreen)
{
    ...
    static TabItem_t CartBackup = {
        .Widget = {
            .Name = "CART BACKUP",
            .fnDraw = CartBackupUI_Draw,
            .fnOnButton = CartBackupUI_OnButton,
            .fnOnTransition = CartBackupUI_OnTransition,
        },
    };

    if ((eResult = Tab_AddItem(pList, &CartBackup, pScreen)) != kOSD_Result_Ok)
    {
        ESP_LOGE(TAG, "%s tab item init failed %d", CartBackup.Widget.Name,
                 eResult);
        return;
    }

    static TabItem_t PCBackupMode = {
        .Widget = {
            .Name = "C. MAGICIAN",
            .fnDraw = PCBackupMode_Draw,
            .fnOnButton = PCBackupMode_OnButton,
            .fnOnTransition = PCBackupMode_OnTransition,
        },
    };

    if ((eResult = Tab_AddItem(pList, &PCBackupMode, pScreen)) !=
        kOSD_Result_Ok)
    {
        ESP_LOGE(TAG, "%s tab item init failed %d",
                 PCBackupMode.Widget.Name, eResult);
        return;
    }
...
static void CreateMenuBackups(void)
{
    static MenuTab_t Backups = {
        .Widget = {
            .Name = "BACKUPS",
            .fnDraw = RomBrowserUI_Draw,
            .fnOnButton = RomBrowserUI_OnButton,
            .fnOnTransition = RomBrowserUI_OnTransition,
        },
        .pImageDesc = &menu_system,
    };
    Backups.Accent = lv_color_make(0x66, 0xFF, 0x66);
    const OSD_Result_t Result = MenuMgr_AddTab(kTabID_Backups, &Backups);
    if (Result != kOSD_Result_Ok)
    {
        ESP_LOGE(TAG, "BACKUPS menu init failed %d", Result);
    }
}
