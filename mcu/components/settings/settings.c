...
static const char* NVS_KeyStr[kNumSettingKeys] = {
    ...
    [kSettingKey_PCBackupMode]     = "pc_backup",
};
...
OSD_Result_t Settings_Commit(void)
{
    if (_Handle == kNoHandle)
    {
        return kOSD_Result_Err_SettingsNVSNotLoaded;
    }
    const esp_err_t Result = nvs_commit(_Handle);
    if (Result != ESP_OK)
    {
        ESP_LOGE(TAG, "Commit failed: %s", esp_err_to_name(Result));
        return kOSD_Result_Err_SettingUpdateFailed;
    }
    return kOSD_Result_Ok;
}
