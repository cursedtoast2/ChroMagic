...
static void notify_lvgl_flush_ready(spi_transaction_t *pTransaction)
{
    ...
    if ((pTransaction->cmd & 0x400) == 0 ||
        pTransaction->addr >= 0x00020000u)
    {
        return;
    }
...
static void example_lvgl_flush_cb(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_map)
{
    if (VirtualCart_IsActive() && !OSD_IsVisible())
    {
...
static void send_lines(spi_device_handle_t spi, int ypos, uint16_t *linedata)
{
    ...
    if (!CartBulk_DisplayTransferBegin())
    {
        lv_disp_flush_ready(&disp_drv);
        return;
    }
    ...
    for (x=0; x<NUMTRANS; x++) {
        ...
        assert(ret==ESP_OK);
        CartBulk_DisplayTransactionQueued();
    }

    CartBulk_DisplayTransferEnd();
...
static void persist_storage_init(void)
{
    ...
    const fnSettingApply_t fnApplySetting[kNumSettingKeys] = {
        ...
        [kSettingKey_PCBackupMode]     = PCBackupMode_ApplySetting,
    };
...
void app_main(void)
{
    ...
    Mutex_Init();
    ESP_ERROR_CHECK(CartLink_Init());
    ESP_ERROR_CHECK(CartBackup_Init());
    ESP_ERROR_CHECK(RomBrowser_Init());
    ESP_ERROR_CHECK(PCBackup_Init());
    ...
    ESP_ERROR_CHECK(ret);
    ESP_ERROR_CHECK(CartBulk_Init(spi));
    PCBackup_TransportReady();
    ...
    ReplHWConfig.baud_rate = PCBackup_GetConsoleBaud();

    ESP_ERROR_CHECK(esp_console_new_repl_uart(&ReplHWConfig, &ReplConfig, &pRepl));
    ESP_ERROR_CHECK(PCBackup_ConfigureConsoleInput());
    esp_console_register_help_command();
    ESP_ERROR_CHECK(SDCard_RegisterConsoleCommand());
    ESP_ERROR_CHECK(FPGA_Rx_RegisterConsoleCommand());
    ESP_ERROR_CHECK(CartBulk_RegisterConsoleCommand());
    ESP_ERROR_CHECK(CartBackup_RegisterConsoleCommand());
    ESP_ERROR_CHECK(VirtualCart_RegisterConsoleCommands());
    ESP_ERROR_CHECK(PCBackup_RegisterConsoleCommand());
    // All commands must be registered prior to starting the REPL
    ESP_ERROR_CHECK(esp_console_start_repl(pRepl));
    PCBackup_ConsoleReady();
