...
typedef enum {
    ...
    kTxCmd_ReqBGPD          = 0xD,
    kTxCmd_CartMaintenance  = 0xE,
    kTxCmd_CartStreamAck    = 0x10,
...
void FPGA_TxTask(void *arg)
{
    ...
    while (1)
    {
        ...
        if ((EventBits & kTxFlag_SetSysCtl) == kTxFlag_SetSysCtl)
        {
            ...
            const uint16_t ismuted        = (uint8_t)(SilentMode_GetState() == kSilentModeState_On || PCBackup_IsEnabled());
...
esp_err_t FPGA_Tx_PulseButtons(uint16_t Buttons)
{
    ...
    const uint16_t States[] = {Buttons, 0};
    for (size_t i = 0; i < sizeof(States) / sizeof(States[0]); ++i)
    {
        uint8_t TxBuffer[7] = {0};
        uint16_t State = States[i];
        const size_t Size = SetupTxBuffer(TxBuffer, kTxCmd_PokeButton,
                                          sizeof(State), &State);
        if (Size == 0 ||
            uart_write_bytes(UART_NUM_1, TxBuffer, Size) != (int)Size)
        {
            return ESP_FAIL;
        }
        ESP_RETURN_ON_ERROR(
            uart_wait_tx_done(UART_NUM_1, pdMS_TO_TICKS(20)), TAG,
            "button pulse UART");
        vTaskDelay(1);
    }
    return ESP_OK;
...
{
    const esp_err_t Result = FPGA_Tx_QueueCartRequest(Request);
    return Result == ESP_OK
        ? uart_wait_tx_done(UART_NUM_1, pdMS_TO_TICKS(20)) : Result;
}
...
esp_err_t FPGA_Tx_QueueCartRequest(const uint8_t Request[8])
{
    ...
    uint8_t RequestCopy[8];
    memcpy(RequestCopy, Request, sizeof(RequestCopy));
    const size_t Size = SetupTxBuffer(TxBuffer, kTxCmd_CartMaintenance,
                                      sizeof(RequestCopy), RequestCopy);
...
esp_err_t FPGA_Tx_CartStreamAck(uint8_t Sequence)
{
    ...
    const size_t Size = SetupTxBuffer(TxBuffer, kTxCmd_CartStreamAck,
...
static size_t SetupTxBuffer(uint8_t *const pBuffer, TxIDs_t eID, uint8_t Len, void* pData)
{
    ...
    const bool UseV1 = FPGA_IsProtoV1() &&
                       eID != kTxCmd_CartMaintenance &&
                       eID != kTxCmd_CartStreamAck;
    pBuffer[0] = (uint8_t)(UseV1 ? kSysMgmtConsts_HeaderV1Marker : kSysMgmtConsts_HeaderV2Marker);
