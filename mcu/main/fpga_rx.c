...
enum { kRxTraceSize = 64 };
static uint8_t RxTrace[kRxTraceSize];
static uint32_t RxTracePosition;
static portMUX_TYPE RxTraceLock = portMUX_INITIALIZER_UNLOCKED;
...
static void PrintRxTrace(const char *pPrefix)
{
    uint8_t Trace[kRxTraceSize];
    const uint32_t End = FPGA_Rx_GetTracePosition();
    const uint32_t Start = End > sizeof(Trace) ? End - sizeof(Trace) : 0;
    const size_t Length = FPGA_Rx_CopyTraceSince(Start, Trace, sizeof(Trace));
    printf("%s end=%lu bytes=%u data=", pPrefix, (unsigned long)End,
           (unsigned)Length);
...
{
    uint32_t Position;
    portENTER_CRITICAL(&RxTraceLock);
    Position = RxTracePosition;
    portEXIT_CRITICAL(&RxTraceLock);
    return Position;
}
...
size_t FPGA_Rx_CopyTraceSince(uint32_t StartPosition, uint8_t *pOutput,
                              size_t Capacity)
{
    if (pOutput == NULL || Capacity == 0)
    {
        return 0;
    }
    portENTER_CRITICAL(&RxTraceLock);
    uint32_t Available = RxTracePosition - StartPosition;
    if (Available > kRxTraceSize)
    {
        StartPosition = RxTracePosition - kRxTraceSize;
        Available = kRxTraceSize;
    }
    if (Available > Capacity)
    {
        StartPosition += Available - Capacity;
        Available = Capacity;
    }
    for (uint32_t i = 0; i < Available; ++i)
    {
        pOutput[i] = RxTrace[(StartPosition + i) % kRxTraceSize];
    }
    portEXIT_CRITICAL(&RxTraceLock);
    return Available;
}

static int ConsoleRxTrace(int argc, char **argv)
{
    (void)argc;
    (void)argv;
    PrintRxTrace("FPGA_RX_TRACE");
    return 0;
}
...
esp_err_t FPGA_Rx_RegisterConsoleCommand(void)
{
    const esp_console_cmd_t Command = {
        .command = "fpgarxtrace",
        .help = "Print the last 64 raw bytes received from the FPGA",
        .hint = NULL,
        .func = ConsoleRxTrace,
        .argtable = NULL,
    };
...
void FPGA_RxTask(void *arg)
{
    ...
    while (1)
    {
        ...
        int32_t ByteCount = uart_read_bytes(UART_NUM_1, pRxBuffer, 1,
                                            pdMS_TO_TICKS(10));

        size_t BufferedBytes = 0;
        if (ByteCount > 0 &&
            uart_get_buffered_data_len(UART_NUM_1, &BufferedBytes) == ESP_OK &&
            BufferedBytes > 0)
        {
            const size_t Capacity = sizeof(RxBuffer) - (size_t)ByteCount;
            const size_t ToRead = BufferedBytes < Capacity
                ? BufferedBytes : Capacity;
            const int32_t Additional = uart_read_bytes(
                UART_NUM_1, &pRxBuffer[ByteCount], ToRead, 0);
            if (Additional > 0)
            {
        ...
        if (ByteCount > 0)
        {
            ...
            {
                RxTrace[RxTracePosition % kRxTraceSize] = pRxBuffer[i];
                ++RxTracePosition;
            }
            portEXIT_CRITICAL(&RxTraceLock);
    ...
    switch (_eState)
    {
        ...
        case kWaitForV2CRC:
            ...
            else
            {
                ...
                if (DecodeBuffer[1] == kRxCmd_CartMaintenance)
                {
                    printf("CARTLINK_RESPONSE_CRC_FAIL length=%lu\n",
...
static void ProcessMessage(const RxMsg_t *const pMsg)
{
    ...
    switch(pMsg->Addr)
    {
        ...
        case kRxCmd_CartMaintenance:
            CartLink_OnResponse(pMsg->Payload, pMsg->Len);
            break;
        case kRxCmd_CartStream:
            CartBulk_OnStreamEvent(pMsg->Payload, pMsg->Len);
            break;
        default:
