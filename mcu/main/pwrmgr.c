...
static portMUX_TYPE SleepInhibitLock = portMUX_INITIALIZER_UNLOCKED;
static uint32_t SleepInhibitCount;
...
static bool IsSleepInhibited(void)
{
    bool Inhibited;
    portENTER_CRITICAL(&SleepInhibitLock);
    Inhibited = SleepInhibitCount != 0;
    portEXIT_CRITICAL(&SleepInhibitLock);
    return Inhibited;
}
...
void PwrMgr_InhibitSleep(void)
{
    bool FirstInhibitor;
    portENTER_CRITICAL(&SleepInhibitLock);
    FirstInhibitor = SleepInhibitCount == 0;
    ++SleepInhibitCount;
    portEXIT_CRITICAL(&SleepInhibitLock);
    if (FirstInhibitor && IdleSystemTimer != NULL)
    {
        (void)xTimerStop(IdleSystemTimer, 0);
    }
}

void PwrMgr_AllowSleep(void)
{
    bool LastInhibitor = false;
    portENTER_CRITICAL(&SleepInhibitLock);
    if (SleepInhibitCount > 0)
    {
        --SleepInhibitCount;
        LastInhibitor = SleepInhibitCount == 0;
    }
    portEXIT_CRITICAL(&SleepInhibitLock);
    if (LastInhibitor && IdleSystemTimer != NULL)
    {
        (void)xTimerReset(IdleSystemTimer, 0);
    }
}
