...
module uart_packet_wrapper_tx(
    ...
    localparam [6:0] CART_UART_GUARD_CYCLES = 7'd80;
    reg [6:0] cart_uart_idle_count = CART_UART_GUARD_CYCLES;
    always @(posedge clk or posedge reset)
    begin
        if (reset || uart_tx_busy)
            cart_uart_idle_count <= 7'd0;
        else if (cart_uart_idle_count < CART_UART_GUARD_CYCLES)
            cart_uart_idle_count <= cart_uart_idle_count + 1'b1;
    end
    wire stock_byte_slot = (~uart_tx_busy) && (cnt == 'd0);
    wire cart_byte_slot = (~uart_tx_busy) &&
                          (cart_uart_idle_count == CART_UART_GUARD_CYCLES);
    wire maintenance_channel = (tx_address == 7'h0a) ||
                               (tx_address == 7'h0d);
    wire byte_slot = maintenance_channel ? cart_byte_slot : stock_byte_slot;
                ...
                case(tx_state)
                    ...
                    TX_IDLE, TX_SLEEP:
                        if(~uart_tx_busy&&write) begin
                            if (menuDisabled || tx_state == TX_SLEEP) begin
                                tx_state <= TX_AWAKE;
                                bytecount <= 8'd20;
                            end else
                                tx_state <= TX_ADDR;
                        end else if (menuDisabled) begin
                            tx_state <= TX_SLEEP;
                        end
    ...
    begin
        ...
        if(~uart_tx_busy&~uartDisabled) begin
            if(tx_state == TX_IDLE | tx_state == TX_SLEEP) begin
                ...
                if(byte_slot && (tx_state != TX_DONE)) begin
                    uart_tx_val <=  1'd1;
                end
            end
        end
    ...
    assign write_done = tx_state == TX_DONE && byte_slot;
    ...
    always @(posedge clk) begin
        if (tx_state == TX_IDLE ||
            (tx_state == TX_START && byte_slot)) begin
            crc <= 8'hFF;
            bit_count <= 4'd0;
