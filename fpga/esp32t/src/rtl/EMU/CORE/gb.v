...
module gb (
   input reset,
    ...
    output cart_rd,
    output cart_wr,
    output cart_wr_pulse,
    input [7:0] cart_do,
    output [7:0] cart_di,
    input  cart_oe,
    input  cart_wait,
    input  virtual_cart_mode,
    input  virtual_quiesce,
    output virtual_quiesced,
    output  [2:0] TSTATEo,
    output  TSTATE1,
    output  TSTATE2,
    output  TSTATE3,
...
wire dma_cart_wait = cart_wait & DMA_on;
wire ce_cpu_dma_ready = ce_cpu & ~dma_cart_wait;
wire cpu_cart_owned_by_dma = virtual_cart_mode && DMA_on &&
                            (sel_rom || sel_cram) && !sel_boot_rom;
wire cpu_cart_wait = (cart_wait && !DMA_on) || cpu_cart_owned_by_dma;

assign virtual_quiesced = virtual_quiesce &&
    (TSTATEo == 3'd2 || (cpu_stop && cpu_rd_n && cpu_wr_n)) &&
    !DMA_on && !cart_wait;
...
always @(posedge clk_sys) begin
    ...
    else if (ce_cpu && !cpu_cart_owned_by_dma) old_cpu_wr_n <= cpu_wr_n;
end
...
GBse cpu (
    ...
    .CLK_n             ( clk_sys         ),
    .CLKEN             ( cpu_clken       ),
    .WAIT_n            ( !(cpu_cart_wait || (virtual_quiesce && TSTATEo == 3'd2)) ),
    .INT_n             ( irq_n           ),
    .NMI_n             ( 1'b1            ),
    .BUSRQ_n           ( 1'b1            ),
   .M1_n              ( cpu_m1_n        ),
...
wire vram_write_ready = (hdma_rd && isGBC) ? !dma_cart_wait : cpu_wr_vram;
wire vram_wren = video_rd?1'b0:!vram_bank&&vram_write_ready;
wire vram1_wren = video_rd?1'b0:vram_bank&&vram_write_ready;
...
wire hdma_vram_wait = virtual_cart_mode && hdma_rd && video_rd;
wire hdma_cpu_write = sel_hdma && !cpu_wr_n_edge;
wire hdma_cpu_bus_wait = virtual_cart_mode && !hdma_rd &&
                        (sel_rom || sel_cram) && !sel_boot_rom &&
                        (!cpu_rd_n || !cpu_wr_n);
(* syn_keep = 1 *) wire hdma_ce = (ce_cpu_dma_ready && !hdma_vram_wait && !hdma_cpu_bus_wait)
                              || (ce_cpu && hdma_cpu_write);
...
assign cart_wr_pulse = cart_sel & ~cpu_wr_n_edge &
